...
public class GoogleDriveOAuthServiceImpl implements GoogleDriveOAuthService {
    ...
    public GoogleDriveOAuthServiceImpl(
            ...
            @Value("${google.oauth2.drive-redirect-uri}")
            String redirectUri
    ) {
    ...
    public GoogleUserInfoResponse getUserInfo(String accessToken) {
        ...
        if (accessToken == null || accessToken.isBlank()) {
            throw new IllegalArgumentException(
                    "Google access token must not be empty"
            );
        }

        GoogleUserInfoResponse userInfo = restClient
                .get()
        ...
        if (userInfo == null) {
            throw new IllegalStateException(
                    "Google returned an empty user info response"
            );
        }
