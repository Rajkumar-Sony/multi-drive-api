...
@Service
public class DriveDownloadServiceImpl implements DriveDownloadService {
	...
	private final GoogleDriveContentStreamingService googleDriveContentStreamingService;

	private final DriveOperationCapabilityGuard driveOperationCapabilityGuard;

	private final DriveExportFormatRegistry driveExportFormatRegistry;

	public DriveDownloadServiceImpl(GoogleDriveItemRepository googleDriveItemRepository,
		...
		this.googleDriveContentStreamingService = googleDriveContentStreamingService;

		this.driveOperationCapabilityGuard = driveOperationCapabilityGuard;

		this.driveExportFormatRegistry = driveExportFormatRegistry;
	}

	@Override
	public DriveContentStreamResponse prepareDownload(String googleSubjectId, Long itemId) {
		...
		GoogleDriveFileResponse remoteFile = googleDriveFileMutationService.getFile(connection.getId(), userId,
				item.getGoogleFileId());

		driveOperationCapabilityGuard.requireAllowed(
				remoteFile.capabilities() != null ? remoteFile.capabilities().canDownload() : null, "DOWNLOAD");

		String remoteMimeType = normalizeContentType(remoteFile.mimeType());

		if (driveExportFormatRegistry.isGoogleNativeMimeType(remoteMimeType)) {

			throw new DriveContentNotSupportedException(
					"Google Workspace files must be exported using the export endpoint");
		}

		String fileName = normalizeFileName(remoteFile.name(), item.getName());

		Long contentLength = parseSize(remoteFile.size());

		Long connectionId = connection.getId();

		String googleFileId = item.getGoogleFileId();
		...
		return new DriveContentStreamResponse(
				...
				outputStream -> googleDriveContentStreamingService.streamBlob(connectionId, userId, googleFileId,
						outputStream));
	}

	@Override
	public DriveContentStreamResponse prepareExport(String googleSubjectId, Long itemId, String exportMimeType) {
		...
		driveOperationCapabilityGuard.requireAllowed(
				remoteFile.capabilities() != null ? remoteFile.capabilities().canDownload() : null, "EXPORT");

		String sourceMimeType = remoteFile.mimeType();

		DriveExportFormatResponse format = driveExportFormatRegistry.resolve(sourceMimeType, exportMimeType);

		String sourceName = normalizeFileName(remoteFile.name(), item.getName());

		String exportedFileName = applyExtension(sourceName, format.extension());
		...
		return new DriveContentStreamResponse(
				...
				outputStream -> googleDriveContentStreamingService.streamExport(connectionId, userId, googleFileId,
						format.mimeType(), outputStream));
	}

	@Override
	public DriveExportOptionsResponse getExportOptions(String googleSubjectId, Long itemId) {
		...
		return new DriveExportOptionsResponse(
				...
				driveExportFormatRegistry.getFormats(mimeType));
	}
	...
	private String normalizeFileName(String remoteName, String localName) {
		...
		else if (localName != null && !localName.isBlank()) {
