...
@Service
public class GoogleDriveWatchServiceImpl
        implements GoogleDriveWatchService {

    private static final String GOOGLE_CHANGES_WATCH_URL =
    ...
    /*
     * Google currently allows changes.watch channels
     * for a maximum of 7 days.
     *
     * We request 6 days and later renew them before expiry.
     */
    private static final Duration WATCH_CHANNEL_LIFETIME =
            Duration.ofDays(6);

    /*
     * If a channel still has more than 12 hours remaining,
     * we reuse it instead of creating another channel.
     */
    private static final Duration REUSE_THRESHOLD =
            Duration.ofHours(12);
    ...
    public GoogleDriveWatchServiceImpl(
            GoogleDriveChangeTrackerRepository
                    googleDriveChangeTrackerRepository,
            ...
            @Value("${google.drive.webhook-url:}")
            String webhookUrl
    ) {
    ...
    @Override
    public List<GoogleDriveWatchChannel> registerWatchChannels(
            Long connectionId,
            Long userId
    ) {
        ...
        /*
         * Security check:
         * connection must belong to this application user.
         */
        googleDriveConnectionRepository
                .findByIdAndUserId(
                        connectionId,
        ...
        List<GoogleDriveChangeTracker> trackers =
                googleDriveChangeTrackerRepository
        ...
        if (trackers.isEmpty()) {
            ...
            throw new IllegalStateException(
                    "No active Google Drive change trackers found"
            );
        }
        ...
        List<GoogleDriveWatchChannel> channels =
                new ArrayList<>();
    ...
    private GoogleDriveWatchChannel registerWatchChannel(
        ...
        if (tracker.getPageToken() == null
            ...
            throw new IllegalStateException(
                    "Change tracker page token is missing"
            );
        }

        Optional<GoogleDriveWatchChannel> reusableChannel =
                findReusableActiveChannel(
                        tracker.getId()
                );
        ...
        String rawChannelToken =
                generateChannelToken();

        String channelTokenHash =
                hashChannelToken(
                        rawChannelToken
                );

        long requestedExpirationMillis =
                Instant.now()
                        .plus(
                                WATCH_CHANNEL_LIFETIME
                        )
                        .toEpochMilli();

        GoogleDriveWatchRequest request =
                new GoogleDriveWatchRequest(
                        channelId,
                        "web_hook",
                        webhookUrl.trim(),
                        rawChannelToken,
                        String.valueOf(
                                requestedExpirationMillis
                        )
                );
        ...
        GoogleDriveWatchResponse response =
                restClient
                        .post()
                        .uri(uri)
                        .header(
                        ...
                        .retrieve()
                        .body(
                                GoogleDriveWatchResponse.class
                        );
        ...
        /*
         * We store only a SHA-256 hash of the
         * channel verification token.
         *
         * The raw token is sent only to Google.
         */
        channel.setChannelToken(
                channelTokenHash
        );
        ...
        return googleDriveWatchChannelRepository
                .save(
                        channel
                );
    }

    private URI buildWatchUri(
            GoogleDriveChangeTracker tracker
    ) {
    ...
    private Optional<GoogleDriveWatchChannel>
    findReusableActiveChannel(
            Long trackerId
    ) {

        LocalDateTime reuseAfter =
                LocalDateTime
                        .now(
                                ZoneOffset.UTC
                        )
        ...
        return googleDriveWatchChannelRepository
                ...
                .max(
                        Comparator.comparing(
                                GoogleDriveWatchChannel::getExpiration
                        )
                );
    }

    private void expireOldChannels(
            Long trackerId
    ) {
    ...
    private void validateWatchResponse(
            GoogleDriveWatchResponse response,
        ...
        if (response == null) {
            ...
            throw new IllegalStateException(
                    "Google Drive returned an empty watch response"
            );
        }
        ...
        if (response.id() == null
            ...
            throw new IllegalStateException(
                    "Google Drive watch channel ID is missing"
            );
        }

        if (!requestedChannelId.equals(
                response.id()
        )) {

            throw new IllegalStateException(
                    "Google Drive returned an unexpected channel ID"
            );
        }

        if (response.resourceId() == null
                || response.resourceId().isBlank()) {

            throw new IllegalStateException(
                    "Google Drive watch resource ID is missing"
            );
        }
    }
    ...
    private long parseExpiration(
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Google Drive returned an invalid channel expiration",
                    exception
            );
        }
    }
    ...
    private void validateWebhookUrl() {
        ...
        if (webhookUrl == null
            ...
            throw new IllegalStateException(
                    "Google Drive webhook URL is not configured"
            );
        }
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Google Drive webhook URL is invalid",
                    exception
            );
        }
        ...
        if (!"https".equalsIgnoreCase(
            ...
            throw new IllegalStateException(
                    "Google Drive webhook URL must use HTTPS"
            );
        }
        ...
        if (uri.getHost() == null
            ...
            throw new IllegalStateException(
                    "Google Drive webhook URL must contain a valid host"
            );
        }
    }
